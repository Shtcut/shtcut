...
const TabsFilter: React.FC<TabsProps> = ({ tabs, selectedTabIndex, onTabClick, classNames, onChange }) => {
    return (
        <div className={`w-full ${classNames}`}>
            ...
            >
                <div
                    className="absolute top-1/2 left-[2px] bg-primary-0  rounded-[3px] transition-all duration-500 ease-in-out shadow-[2px_0_8px_rgba(167,169,174,0.15)]"
                    style={{
                        height: 'calc(100% - 2px)',
                ...
                {tabs.map((tab, index) => (
                    <button
                        key={tab.id}
                        className={cn(
                            'relative border-[0.5px] border-custom-border-200 rounded  z-[1] font-semibold text-xs  py-1.5 focus:outline-none transition duration-500',
                            {
