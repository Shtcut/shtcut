...
export const CustomAlert = (props: CustomAlertProps) => {
    const { title, description, confirmMessage, action, close } = props;

    return (
        <AlertDialog.Portal>
            <AlertDialog.Overlay className="fixed inset-0 backdrop-blur-sm bg-gary-800 bg-opacity-50 sm:w-full" />
            <AlertDialog.Content className="contentShow fixed top-[50%] left-[50%] max-h-[85vh] w-[90vw] max-w-[500px] translate-x-[-50%] translate-y-[-50%] rounded-xl bg-white p-[25px] ">
                <AlertDialog.Title className="text-slate-900 m-0 text-[17px] font-medium">{title}</AlertDialog.Title>
